...
def main():
    ...
    # Charger le fichier txt afin d'ajouter la colonne text parce qu'elle manque dans le dataset ( il y a que la colonne audio mais pas la transcription)
    df_text = pd.read_csv("data_arab_tunisien.txt", sep="\t")
    local_texts = df_text["tgt_text_without_diacritization"].tolist()
    if len(local_texts) < len(raw_datasets["train"]):
        local_texts.extend([""] * (len(raw_datasets["train"]) - len(local_texts)))
    elif len(local_texts) > len(raw_datasets["train"]):
        local_texts = local_texts[:len(raw_datasets["train"])]
    
    # Vérification des tailles après ajustement
    print(f"Nombre de textes locaux après ajustement : {len(local_texts)}")
    print(f"Nombre d'exemples dans le dataset : {len(raw_datasets['train'])}")
    raw_datasets["train"] = raw_datasets["train"].map(lambda example, idx: {"tgt_text_without_diacritization": local_texts[idx]}, with_indices=True)
    if training_args.do_eval:
        ...
        # Ajouter la colonne "tgt_text" à l'évaluation aussi
        local_texts_eval = df_text["tgt_text"].tolist()  # Assumes df_text is large enough for eval
        if len(local_texts_eval) < len(raw_datasets["eval"]):
            local_texts_eval.extend([""] * (len(raw_datasets["eval"]) - len(local_texts_eval)))
        elif len(local_texts_eval) > len(raw_datasets["eval"]):
            local_texts_eval = local_texts_eval[:len(raw_datasets["eval"])]
    
        raw_datasets["eval"] = raw_datasets["eval"].map(lambda example, idx: {"tgt_text": local_texts_eval[idx]}, with_indices=True)
    
    # Afficher un exemple de dataset pour vérifier
